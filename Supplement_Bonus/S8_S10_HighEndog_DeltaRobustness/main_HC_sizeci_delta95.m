% Table_Est_HC replica at a MILDER heteroskedasticity design: delta ~ 0.95 (HET_B=0.2265),
% proper Fuller sandwich. Same DGP as main_HC_sizeci_delta78.m except HET_B, backing the
% main-text footnote "we also ran the experiment at other values of delta, both milder and
% more severe; the conclusions reported below are unchanged."
clear;
global HET_TYPE HET_Z HET_B
rng(20260628);
load X2.mat;
HET_TYPE = 1;
HET_B    = 0.2265;                 % delta = exp(-HET_B^2) ~ 0.95
HET_Z    = randn(200,1);
config.eq           = 1;
...
for k2 = [1 3 5]
    ...
    movefile(d(ix).name, sprintf('sizeci95_L%d.mat', k2-1));
    fprintf('done L=%d\n', k2-1);
end
v = exp(HET_B*HET_Z); v = v/mean(v);
fprintf('design delta = %.4f\n', (mean(v))^2/mean(v.^2));
disp('SIZECI95_RUN_DONE');
